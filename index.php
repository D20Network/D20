...
$time_start = microtime(true); 
  ...
  $MAIN_ENDPOINT = "http://127.0.0.1:17461/";
  $TEST_ENDPOINT = "http://127.0.0.1:17461/";
  define("BASE_URL", "/explorer/"); #set to absolute URL, use "/" if in root directory
  define("USE_MOD_REWRITE", false);
	define("RPC_USER", "rpc_d20network");
	define("RPC_PASS", "dR2oBQ3K1zYMZQtJFZeAerhWxaJ5Lqeq9J2");
	define("BLOCK_TXDETAILS", false); #blocks load faster without txdetails
  ...
  $FOOTER = '<a href="https://github.com/jonasschnelli/simple-block-explorer">Simple Block Explorer</a> for D20 Network Core';
...
###### END CONFIG SECTION
  ...
  $VIEWTYPE = ViewType::Overview;
	if(isset($_REQUEST['txhistory'])) {
		...
		if (count($blocks) == 0) {
			...
			echo "<div>Balance: ".D20amount($txhistorybalance)."</div>\n";
      echo '<ul class="list-group list-numbered my-2">'."\n";
        ...
        echo '<li class="list-group-item d-flex justify-content-between lh-condensed">';
				...
				echo $tx['txid'];
				echo '	</div>';
				echo '	<div><strong>'.D20amount($tx['value']).'</strong></div>';
				echo '</li>';
			}
			echo "</ul>\n";
			...
			if (isset($_REQUEST['utxolookupstatus'])) {
				...
				if(is_null($data->result)) {
					...
					if(!is_null($data->result)) {
						...
						echo "<hr /><div class=\"totalamount\"><b>Total unspent D20:</b> ".$data->result->total_amount."</div><br>\n";
						...
						foreach($data->result->unspents as $utxo) {
							echo "<tr>\n";
							echo "<td>".D20amount($utxo->amount)."</td>\n";
  ...
  function D20amount($amount) {
    return number_format($amount, 8, ".", "'")." D20";
  }
	...
	<nav class="navbar bg-dark shadow navbar-expand-lg sticky-top bg-body-tertiary" data-bs-theme="dark">
	  <div class="container-fluid">
				...
				<a class="navbar-brand" href="<?php echo homelink(); ?>">
					<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="D20 Networklogo me-2" viewBox="0 0 64 64">
	    ...
	    <div class="collapse navbar-collapse" id="navbarSupportedContent">
	      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
	        <li class="nav-item">
						<a class="nav-link <?php if($ENDPOINT == $MAIN_ENDPOINT) echo "active"; ?>" href="<?php echo overviewlink(); ?>" aria-current="page" >D20 Network</a>
	        </li>
	        <li class="nav-item">
	      ...
	      <ul class="navbar-nav flex-row flex-wrap ms-md-auto">
	        ...
	        <li class="nav-item py-1 px-0 px-lg-2">
						<a href="https://github.com/d20network/d20" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#fff" class="bi bi-github" viewBox="0 0 16 16">
			  ...
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
			      <div class="fw-bold">Total amount in all outputs</div>
						<?php echo D20amount($block->stats->total_out/100000000.0); ?>
			    </div>
			  </li>
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
			      <div class="fw-bold">Total fees</div>
						<?php echo D20amount($block->stats->totalfee/100000000); ?>
			    </div>
			  </li>
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
			      <div class="fw-bold">Total fees & blockreward</div>
						<?php echo D20amount(($block->stats->totalfee+$block->stats->subsidy)/100000000); ?>
			    </div>
			  </li>
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
			  ...
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
			      <div class="fw-bold">Feerange</div>
						<?php echo $block->stats->minfeerate; ?>-<?php echo $block->stats->maxfeerate; ?> shards/vB
			    </div>
			  </li>
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
			      <div class="fw-bold">Medianfee</div>
						<?php echo $block->stats->medianfee; ?> shards
			    </div>
			  </li>
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
			  ...
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
			      <div class="fw-bold">Feerate</div>
						<?php echo round((isset($tx->mempool) ? $tx->mempool->fees->base : $tx->fee)*100000000/$tx->vsize, 2); ?> shards/vB
			    </div>
			  </li>
			  <li class="list-group-item d-flex justify-content-between align-items-start">
			    <div class="ms-2 me-auto">
...
<?php if (isset($vin->prevout)):?>
            <div class="text-truncate"><a href="<?php echo txlink($vin->txid, $vin->vout); ?>"><?php echo shortAddr($vin->prevout->scriptPubKey->address); ?><br><span class="small_vin">txid: <?php echo shortHash($vin->txid)." vout:".$vin->vout; ?></span></a></div><span class="text-muted"><?php echo D20amount($vin->prevout->value); ?></span>
...
<?php elseif($VIEWTYPE == ViewType::TxHistory): ?>
  <div class="row">
    <div class="col">
			...
			<div>Balance: <?php echo D20amount($txhistorybalance); ?></div>
      <ul class="list-group list-numbered my-2">
        ...
        <li class="list-group-item d-flex justify-content-between lh-condensed">
					...
					<div><strong><?php echo D20amount($tx['value']); ?></strong></div>
				</li>
				<?php endforeach;?>
			</ul>
		<?php endif; ?>
